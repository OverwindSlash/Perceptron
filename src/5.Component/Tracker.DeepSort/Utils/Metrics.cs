...
using Tracker.DeepSort.Extensions;
using Vector = Tracker.DeepSort.Utils.DataStructs.Vector;

namespace Tracker.DeepSort.Utils;
...
public static class Metrics
{
    ...
    public static float CosineSimilarity(Vector first, Vector second) => first.Dot(second) / (first.Magnitude * second.Magnitude);

    public static float CosineDistance(Vector first, Vector second) => 1 - CosineSimilarity(first, second);
    ...
    public static Vector BoundsDeltaVelocity(RectangleF first, RectangleF second)
    {
        Vector firstCenter = new Vector(first.X + first.Width / 2, first.Y + first.Height / 2);
        Vector secondCenter = new Vector(second.X + second.Width / 2, second.Y + second.Height / 2);
        Vector delta = firstCenter - secondCenter;
        float magnitude = delta.Magnitude + float.Epsilon;

        return delta / magnitude;
    }

    public static float MedianCosineDistance(Vector[] knownAppearances, int appearanceLength, Vector appearance)
    {
