...
namespace Perceptron.Domain.Entity.RegionDefinition.Geometric;
...
public class NormalizedPolygon : ImageBasedGeometric
{
    ...
    public Rect GetBoundingBox()
    {
        ...
        double minX = _points.Min(p => p.OriginalX);
        double maxX = _points.Max(p => p.OriginalX);
        double minY = _points.Min(p => p.OriginalY);
        double maxY = _points.Max(p => p.OriginalY);
